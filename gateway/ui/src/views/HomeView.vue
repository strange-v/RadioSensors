...
<template><section class="empty-state panel"><template v-if="failure"><span class="state-icon warning"><Icon name="alert" /></span><h1>{{ $t('error.title') }}</h1><p>{{ $t(`error.${failure}`) }}</p><button class="button primary" type="button" @click="load">{{ $t('common.retry') }}</button></template><template v-else><span class="loader"></span><p>{{ $t('common.loading') }}</p></template></section></template>
